...
function WhiteLabelAdd({ setOpenModel, fetchWLList }) {
    ...
    async function submitHandle() {
        ...
        try {
            await axiosInstance.post('/client/whitelabel', payload)
            alert('New White Label Added')
            setFormData({})
            setOpenModel(false)
            void fetchWLList()
        } catch (error) {
    ...
    return (
        <div>
            <PageHead title='Add White Label' />
            <Paper elevation={3} sx={{ p: 2, bgcolor: '#D3D3D3', mt: 1 }} >
                <div className='d-flex flex-column gap-2 align-items-start'>
                    <div className='d-flex justify-content-start gap-2 align-items-center' style={{ width: '100%' }}>
                        <label htmlFor="companyName" style={{ width: '150px' }}>Company Name:</label>
                        <input type="text" id="companyName" className='form-control' onChange={changeHandle} value={formData?.companyName || ''} />
                    </div>
                    <div className='d-flex justify-content-start gap-2 align-items-center' style={{ width: '100%' }}>
                        <label htmlFor="name" style={{ width: '150px' }}>Contact Name:</label>
                    ...
                    <div className='d-flex justify-content-start gap-2 align-items-center' style={{ width: '100%' }}>
                        <label htmlFor="gst" style={{ width: '150px' }}>GST Number:</label>
                        <input type="text" id="gst" className='form-control' onChange={changeHandle} value={formData?.gst || ''} />
                    </div>
                    <div className='d-flex justify-content-start gap-2 align-items-center' style={{ width: '100%' }}>
                        <label htmlFor="pan" style={{ width: '150px' }}>PAN:</label>
                        <input type="text" id="pan" className='form-control' onChange={changeHandle} value={formData?.pan || ''} />
                    </div>
                    <div className='d-flex justify-content-start gap-2 align-items-center' style={{ width: '100%' }}>
